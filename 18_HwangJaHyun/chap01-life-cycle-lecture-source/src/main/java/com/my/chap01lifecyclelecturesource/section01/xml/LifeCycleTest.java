package com.my.chap01lifecyclelecturesource.section01.xml;
...
/* * Servlet Class
* (Server + Applet)
* 자바를 이용해서 요청, 응답 처리를 할 수 있게 만든 클래스
*
* * servlet class 를 만드는 방법
* HttpServlet Class를 상속 받아서 필요한 메서드를 오버라이딩
*
* * servlet의 생명 주기 (life cycle)
* - init() : servlet 최초 호출 또는 서버 배포 시
*            해당 클래스를 인스턴스화 하면서 실행되는 메서드
* - service() : 클라이언트 요청에 대한 서비스를 제공하는 메서드
* - destroy() : 서버 재시작 또는 종료 시점에 서블릿 인스턴스가 소멸하면서 실행되는 메서드
*
* * Servlet 등록
* HttpServlet을 상속 받은 클래스를 등록해서
* servlet Contaier가 관리하고, 요청/ 응답 처리를 할 수 있게 만드는 과정
* 1. xml방식
* 2. 어노테이션 방식
* */
